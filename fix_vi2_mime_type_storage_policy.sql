...
-- Этот скрипт обновляет политики и настройки Storage для поддержки файлов .vi2
...
-- 3. Создаем политику для загрузки файлов с расширением .vi2
CREATE POLICY "Allow vi2 file uploads" ON public.storage_objects
FOR INSERT TO public
WITH CHECK (
  bucket_id IN ('qualification-objects', 'documents') AND
  (
    storage.extension(name) IN ('vi2', 'mp4', 'avi', 'mov', 'wmv', 'flv', 'webm') OR
    storage.extension(name) IN ('mp3', 'wav', 'ogg', 'aac', 'flac') OR
    storage.extension(name) IN ('pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'txt', 'rtf') OR
    storage.extension(name) IN ('jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp', 'tiff', 'tif') OR
    storage.extension(name) IN ('zip', 'rar', '7z', 'tar', 'gz') OR
    storage.extension(name) IN ('csv', 'json', 'xml', 'dat', 'bin')
  )
);
...
-- 7. Проверяем текущие настройки bucket
SELECT 
  id as bucket_id,
  name as bucket_name,
  allowed_mime_types,
...
-- 8. Проверяем существующие политики
SELECT 
  schemaname,
  tablename,
  policyname,
...
-- 9. Выводим информацию о выполненных изменениях
SELECT 
  'Политики Storage обновлены для поддержки файлов .vi2 и других типов' as message,
