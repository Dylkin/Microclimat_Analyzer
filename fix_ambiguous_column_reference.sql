-- ИСПРАВЛЕНИЕ ОШИБКИ AMBIGUOUS COLUMN REFERENCE
-- Выполните этот скрипт в SQL Editor в Supabase Dashboard

-- 1. Проверяем существующие таблицы (исправленная версия)
DO $$
DECLARE
    tbl_name text;
...
BEGIN
    ...
    LOOP
        IF EXISTS (
            ...
            RAISE NOTICE 'Таблица % существует', tbl_name;
        ELSE
            RAISE NOTICE 'Таблица % НЕ существует', tbl_name;
        END IF;
    END LOOP;
...
-- Qualification object testing periods
DO $$
BEGIN
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            RAISE NOTICE 'Таблица % не существует, пропускаем создание политик', tbl_name;
        END IF;
    END LOOP;
END $$;
