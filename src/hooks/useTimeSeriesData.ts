...
export const useTimeSeriesData = ({ files, qualificationObjectId, projectId, measurementZones }: UseTimeSeriesDataProps) => {
  ...
  const loadDataFromDatabase = useCallback(async () => {
    ...
    if (measurementZones === undefined) {
      // Ждем, пока зоны измерения будут загружены, чтобы не строить данные без фильтра.
      return;
    }
    ...
    try {
      ...
      const normalizeZone = (zoneNumber: number | string | null | undefined) => (
        zoneNumber === null || zoneNumber === undefined
          ? 0
          : (typeof zoneNumber === 'string' ? parseInt(zoneNumber, 10) : Number(zoneNumber))
      );

      const normalizeLevelStr = (level: number | string | null | undefined) => {
        if (level === null || level === undefined) return null;
        const normalized = typeof level === 'string' ? parseFloat(level) : Number(level);
        if (isNaN(normalized)) return null;
        return normalized.toString();
      };

      const allowedKeys = new Set<string>();
      const allowedLoggerNames = new Set<string>();
      if (measurementZones && measurementZones.length > 0) {
        measurementZones.forEach(zone => {
          zone.measurementLevels.forEach(level => {
            const levelStr = normalizeLevelStr(level.level);
            if (levelStr === null) return;
            allowedKeys.add(`zone-${normalizeZone(zone.zoneNumber)}-level-${levelStr}`);
            if (level.equipmentName && level.equipmentName.trim().length > 0) {
              allowedLoggerNames.add(level.equipmentName.trim());
            }
          });
        });
      }

      const keyFilteredSummaries = allowedKeys.size > 0
        ? summaries.filter(summary => {
            const levelStr = normalizeLevelStr(summary.measurement_level);
            if (levelStr === null) return false;
            return allowedKeys.has(`zone-${normalizeZone(summary.zone_number)}-level-${levelStr}`);
          })
        : summaries;

      const loggerNameFiltered = new Map<string, any>();
      if (allowedLoggerNames.size > 0) {
        summaries.forEach(summary => {
          const loggerName = summary.logger_name?.trim();
          if (!loggerName || !allowedLoggerNames.has(loggerName)) {
            return;
          }
          const existing = loggerNameFiltered.get(loggerName);
          const summaryTime = new Date(summary.end_date || summary.created_at || 0).getTime();
          const existingTime = existing ? new Date(existing.end_date || existing.created_at || 0).getTime() : -1;
          if (!existing || summaryTime >= existingTime) {
            loggerNameFiltered.set(loggerName, summary);
          }
        });
      }

      let filteredSummaries: any[] = [];
      const combinedByLoggerName = new Map<string, { summary: any; source: 'key' | 'name' }>();
      const keyFilteredWithoutLogger: any[] = [];

      if (allowedLoggerNames.size > 0) {
        keyFilteredSummaries.forEach(summary => {
          const loggerName = summary.logger_name?.trim();
          if (!loggerName || !allowedLoggerNames.has(loggerName)) {
            keyFilteredWithoutLogger.push(summary);
            return;
          }
          if (!combinedByLoggerName.has(loggerName)) {
            combinedByLoggerName.set(loggerName, { summary, source: 'key' });
          }
        });

        loggerNameFiltered.forEach((summary, loggerName) => {
          if (!combinedByLoggerName.has(loggerName)) {
            combinedByLoggerName.set(loggerName, { summary, source: 'name' });
          }
        });

        filteredSummaries = [
          ...keyFilteredWithoutLogger,
          ...Array.from(combinedByLoggerName.values()).map(entry => entry.summary)
        ];
      } else {
        filteredSummaries = keyFilteredSummaries;
      }
      ...
      for (const summary of filteredSummaries) {
        ...
        const normalizedZoneNumber = normalizeZone(summary.zone_number);
        
        const normalizedLevelStr = normalizeLevelStr(summary.measurement_level);
        const normalizedLevel = normalizedLevelStr !== null ? parseFloat(normalizedLevelStr) : summary.measurement_level;
        ...
        if (records && records.length > 0) {
          const points: TimeSeriesPoint[] = records.map((record: any, index: number) => {
            ...
            return {
              ...
              fileId: `zone-${normalizedZoneNumber}-level-${normalizedLevelStr ?? summary.measurement_level}`,
              originalIndex: index,
              zoneNumber: normalizedZoneNumber, // Нормализованное значение: null -> 0 для внешних датчиков
              measurementLevel: normalizedLevel,
  ...
  }, [qualificationObjectId, projectId, measurementZones]);
