...
export const QualificationObjectForm: React.FC<QualificationObjectFormProps> = ({
  ...
  const [formData, setFormData] = useState<CreateQualificationObjectData>({
    ...
    storageZones: initialStorageZones,
  ...
  const addStorageZone = () => {
    if (mode === 'view') return;
    setFormData(prev => {
      const zones = prev.storageZones || [];
      const newZone = {
        id: `storage-zone-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
        name: '',
        volume: undefined
      };
      return { ...prev, storageZones: [...zones, newZone] };
    });
  };

  const removeStorageZone = (zoneId: string) => {
    if (mode === 'view') return;
    setFormData(prev => ({
      ...prev,
      storageZones: (prev.storageZones || []).filter(zone => zone.id !== zoneId)
    }));
  };

  const updateStorageZone = (zoneId: string, updates: { name?: string; volume?: number }) => {
    if (mode === 'view') return;
    setFormData(prev => ({
      ...prev,
      storageZones: (prev.storageZones || []).map(zone =>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {formData.type !== 'холодильник' && formData.type !== 'морозильник' && formData.type !== 'термоконтейнер' && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div className="border border-gray-200 rounded-md p-4">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-medium text-gray-900">Зоны хранения</h4>
              {mode !== 'view' && (
                <button
                  type="button"
            ...
            {(formData.storageZones || []).length === 0 ? (
              <div className="text-sm text-gray-500">Зоны хранения не заданы</div>
            ) : (
              <div className="space-y-3">
                {(formData.storageZones || []).map((zone, index) => (
                  <div key={zone.id} className="border border-gray-200 rounded-md p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {mode !== 'view' && (
                        <button
                          type="button"
                          onClick={() => removeStorageZone(zone.id)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          type="text"
                          value={zone.name || ''}
                          onChange={(e) => updateStorageZone(zone.id, { name: e.target.value })}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          min="0"
                          value={zone.volume ?? ''}
                          onChange={(e) => updateStorageZone(zone.id, { volume: e.target.value === '' ? undefined : parseFloat(e.target.value) })}
                          className={`w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm ${
                            mode === 'view' ? 'bg-gray-100 cursor-not-allowed' : ''
                          }`}
                          readOnly={mode === 'view'}
