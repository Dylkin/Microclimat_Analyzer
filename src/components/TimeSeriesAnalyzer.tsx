...
export const TimeSeriesAnalyzer: React.FC<TimeSeriesAnalyzerProps> = ({ files, onBack }) => {
  ...
  // Состояния для генерации отчета
  ...
  const [reportData, setReportData] = useState<ReportData>({
    ...
    nameOfObject: '',
    nameOfAirConditioningSystem: '',
    nameOfTest: 'Испытание климатической установки',
    dateTimeOfTestStart: '',
    dateTimeOfTestCompletion: '',
    durationOfTest: '',
    acceptanceCriteria: '',
    resultsTable: [],
    result: '',
  ...
  const handleGenerateReport = async () => {
    ...
    if (!reportData.reportNo || !reportData.nameOfObject) {
      alert('Пожалуйста, заполните обязательные поля: номер отчета и название объекта');
      return;
    }
    ...
    try {
      // Проверяем шаблон
      const validation = await ReportGenerator.validateTemplate(templateFile);
      if (!validation.isValid) {
        alert(`Ошибки в шаблоне:\n${validation.errors.join('\n')}`);
        return;
      }

      // Подготавливаем данные таблицы результатов
      const tableData = analysisResults.map(result => ({
        zoneNumber: result.zoneNumber.toString(),
        measurementLevel: result.measurementLevel.toString(),
        loggerName: result.loggerName,
        serialNumber: result.serialNumber,
        minTemp: result.minTemp.toString(),
        maxTemp: result.maxTemp.toString(),
        avgTemp: result.avgTemp.toString(),
      ...
      // Получаем SVG элемент графика для вставки в отчет
      const chartSvg = chartRef.current?.querySelector('svg') as SVGSVGElement;

      // Генерируем отчет
      await ReportGenerator.generateReport(templateFile, finalReportData, chartSvg);
  ...
  const downloadPlaceholdersGuide = () => {
    const placeholders = ReportGenerator.getAvailablePlaceholders();
    let content = 'ПЛЕЙСХОЛДЕРЫ ДЛЯ ШАБЛОНА ОТЧЕТА\n\n';
    ...
    content += '1. Создайте документ Word (.docx)\n';
    content += '2. Вставьте нужные плейсхолдеры в места, где должны быть данные\n';
    content += '3. Для таблицы результатов используйте плейсхолдер {Results table}\n';
    content += '4. Для графика используйте плейсхолдер {chart} - изображение будет повернуто на 90° против часовой стрелки\n';
    content += '5. Сохраните документ и загрузите как шаблон\n';

    const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'Плейсхолдеры_для_шаблона.txt';
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleReportDataChange('reportNo', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  placeholder="Например: ОТЧ-001-2025"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleReportDataChange('reportDate', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={reportData.nameOfObject}
                  onChange={(e) => handleReportDataChange('nameOfObject', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  placeholder="Например: Склад фармацевтической продукции"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={reportData.nameOfAirConditioningSystem}
                  onChange={(e) => handleReportDataChange('nameOfAirConditioningSystem', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  placeholder="Например: Система кондиционирования HVAC-001"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Дата и время начала испытания
                </label>
                <input
                  type="text"
                  value={reportData.dateTimeOfTestStart}
                  onChange={(e) => handleReportDataChange('dateTimeOfTestStart', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  placeholder="Например: 01.06.2025 09:00"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={reportData.dateTimeOfTestCompletion}
                  onChange={(e) => handleReportDataChange('dateTimeOfTestCompletion', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  placeholder="Например: 05.06.2025 18:00"
                />
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  value={reportData.acceptanceCriteria}
                  onChange={(e) => handleReportDataChange('acceptanceCriteria', e.target.value)}
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  placeholder="Укажите критерии приемки для испытания..."
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => handleReportDataChange('result', e.target.value)}
                  rows={4}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  placeholder="Укажите выводы по результатам испытания..."
                />
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <button
                ...
                className="bg-purple-600 text-white px-6 py-3 rounded-lg hover:bg-purple-700 transition-colors flex items-center space-x-2 disabled:bg-gray-400 disabled:cursor-not-allowed"
              >
                {generatingReport ? (
                  <>
