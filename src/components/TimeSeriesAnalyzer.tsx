        ...
        {data && (
          <div className={`border rounded-lg p-6 ${
            ...
            <h3 className={`text-lg font-semibold mb-4 ${
              dataType === 'temperature' ? 'text-red-900' : 'text-blue-900'
            }`}>
              {dataType === 'temperature' ? '🌡️' : '💧'} Рекомендации по {dataType === 'temperature' ? 'температуре' : 'влажности'}
            </h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Статистика по текущим данным */}
              <div className={`text-sm ${dataType === 'temperature' ? 'text-red-800' : 'text-blue-800'}`}>
                <h4 className="font-medium mb-2">Текущие показатели:</h4>
                <div className="space-y-1">
                  <div>Диапазон: {dataType === 'temperature' 
                    ? `${data.temperatureRange[0].toFixed(1)}°C - ${data.temperatureRange[1].toFixed(1)}°C`
                    : `${data.humidityRange[0].toFixed(1)}% - ${data.humidityRange[1].toFixed(1)}%`
                  }</div>
                  <div>Разброс: {dataType === 'temperature'
                    ? `${(data.temperatureRange[1] - data.temperatureRange[0]).toFixed(1)}°C`
                    : `${(data.humidityRange[1] - data.humidityRange[0]).toFixed(1)}%`
                  }</div>
                  <div>Всего точек данных: {data.points.length.toLocaleString('ru-RU')}</div>
                </div>
              </div>
            ...
            <div className={`mt-4 p-3 rounded-lg ${
              dataType === 'temperature' ? 'bg-red-100' : 'bg-blue-100'
            }`}>
              <div className={`text-xs ${dataType === 'temperature' ? 'text-red-700' : 'text-blue-700'}`}>
                <strong>Анализ данных:</strong>
                {dataType === 'temperature' ? (
                  <>
                    ...
                    {data.temperatureRange[1] > 50 && (
                      <span className="block mt-1">🔥 Обнаружены высокие температуры - возможны проблемы с охлаждением</span>
                    )}
                  </>
                ) : (
                  <>
                    ...
                    {data.humidityRange[1] - data.humidityRange[0] > 40 && (
                      <span className="block mt-1">⚠️ Большие колебания влажности могут повредить чувствительные материалы</span>
                    )}
                  </>
                )}
                {limits[dataType] && (
                  <span className="block mt-1">✅ Установлены лимиты - проверьте соответствие в таблице результатов</span>
                )}
              </div>
            </div>
          </div>
