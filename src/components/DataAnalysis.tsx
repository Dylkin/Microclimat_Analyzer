    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-6">
          ...
          <div className="pt-4 border-t border-gray-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
              ...
              <div>
                <span className="font-medium text-gray-800">Зоны хранения:</span>
                {selectedQualificationObject.storageZones && selectedQualificationObject.storageZones.length > 0 ? (
                  <div className="text-gray-700 space-y-1">
                    {selectedQualificationObject.storageZones.map((zone) => {
                      const volumeLabel = zone.volume !== undefined && zone.volume !== null ? ` (${zone.volume} м³)` : '';
                      return (
                        <div key={zone.id}>
                          {zone.name && zone.name.trim().length > 0 ? zone.name : 'Без наименования'}
                          {volumeLabel}
                        </div>
                      );
                    })}
                  ...
                  <p className="text-gray-700">Не указаны</p>
                )}
              </div>
        ...
        <TimeSeriesAnalyzer 
          ...
          storageZones={selectedQualificationObject.storageZones || []}
        />
      </div>
    );
  }
