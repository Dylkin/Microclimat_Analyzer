...
export const DataVisualization: React.FC<DataVisualizationProps> = ({ files, onBack }) => {
  ...
  const addVerticalLine = (timestamp: number, chartType: 'temperature' | 'humidity') => {
  ...
  const updateLineComment = (lineId: string, comment: string, chartType: 'temperature' | 'humidity') => {
    if (chartType === 'temperature') {
