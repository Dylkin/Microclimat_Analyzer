...
export class Testo174HParser {
  ...
  private parseMeasurements(textContent: string): MeasurementEntry[] {
    ...
    // Попытка найти табличные данные в тексте
    const tableData = this.parseTableData(textContent);
    
    if (tableData.length > 0) {
      measurements.push(...tableData);
    } else {
      // Если парсинг таблицы не удался, используем тестовые данные
  ...
  private parseTableData(textContent: string): MeasurementEntry[] {
    const measurements: MeasurementEntry[] = [];
    
    try {
      // Поиск строк с данными измерений
      // Формат: id дата/время температура влажность
      ...
      for (const line of lines) {
        ...
        if (!trimmedLine || trimmedLine.includes('id') || trimmedLine.includes('Дата')) {
          continue;
        }
        
        // Попытка парсинга строки с данными
        const match = trimmedLine.match(/^(\d+)\s+(\d{2})\.(\d{2})\.(\d{4})\s+(\d{2}):(\d{2}):(\d{2})\s+([+-]?\d+[,.]?\d*)\s+([+-]?\d+[,.]?\d*)/);
      ...
      console.log(`Извлечено ${measurements.length} записей из текстовых данных`);
    } catch (error) {
      console.error('Ошибка парсинга табличных данных:', error);
    }
