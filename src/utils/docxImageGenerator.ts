...
export class DocxImageGenerator {
  private static instance: DocxImageGenerator;

  static getInstance(): DocxImageGenerator {
    if (!DocxImageGenerator.instance) {
      DocxImageGenerator.instance = new DocxImageGenerator();
    }
    return DocxImageGenerator.instance;
  }
  ...
  /**
   * Генерация DOCX отчета с PNG изображением
   */
  async generateReportWithImage(
    data: ImageReportData, 
    chartElement: HTMLElement
  ): Promise<Blob> {
    try {
      console.log('Создание DOCX отчета с изображением...');
      
      // Создаем скриншот графика с поворотом
      const imageBuffer = await this.createRotatedScreenshot(chartElement);
      console.log('Скриншот создан, размер:', imageBuffer.byteLength, 'байт');
      ...
      console.error('Ошибка генерации DOCX отчета с изображением:', error);
      throw new Error(`Не удалось создать отчет с изображением: ${error instanceof Error ? error.message : 'Неизвестная ошибка'}`);
    }
  }
