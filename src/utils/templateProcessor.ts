...
export class TemplateProcessor {
  ...
  static async processTemplate(templateBuffer: ArrayBuffer, data: TemplateData): Promise<ArrayBuffer> {
    try {
      ...
      const templateData = {
        ...
        CHART: this.createChartDescription(data),
  ...
  /**
   * Создание описания графика с ссылкой на PNG изображение
   */
  private static createChartDescription(data: TemplateData): string {
    const chartDescription = `График временных рядов (${data.DATA_TYPE})

ССЫЛКА НА ИЗОБРАЖЕНИЕ ГРАФИКА:
[CHART_IMAGE_PNG] - Вставьте сюда изображение графика в формате PNG

Инструкция по вставке изображения:
1. Откройте приложение и перейдите к анализу данных
2. Настройте график (тип данных, лимиты, масштаб)
3. Нажмите правой кнопкой мыши на график
4. Выберите "Сохранить изображение как..." или "Копировать изображение"
5. Сохраните в формате PNG
6. Вставьте изображение в документ вместо этого текста

Альтернативный способ:
Используйте функцию "Сформировать отчет" без шаблона - система автоматически создаст отчет с встроенным графиком.

Параметры текущего графика:
- Тип данных: ${data.DATA_TYPE}
- Дата анализа: ${data.DATE}
- Количество файлов: ${data.totalFiles || 'не указано'}`;
