  ...
  static async processTemplate(templateBuffer: ArrayBuffer, data: TemplateData): Promise<ArrayBuffer> {
    try {
      ...
      const templateData = {
        ...
        CHART: `График временных рядов (${data.DATA_TYPE})\n\nПримечание: Автоматическая вставка изображений графиков в DOCX шаблоны не поддерживается в браузерной версии приложения. График необходимо вставить вручную после генерации отчета.\n\nДля получения изображения графика используйте функцию "Сформировать отчет" без шаблона - система создаст стандартный отчет с встроенным графиком.`,
