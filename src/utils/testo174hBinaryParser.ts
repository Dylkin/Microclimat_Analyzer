...
/**
 * Парсер для двухканальных логгеров Testo 174H (температура + влажность)
 */
export class Testo174HBinaryParser {
  ...
  async parse(fileName: string): Promise<ParsedFileData> {
    try {
      console.log('Парсинг двухканального файла Testo 174H:', fileName);
      ...
      console.error('Ошибка парсинга файла Testo 174H:', error);
      return {
        fileName,
        deviceMetadata: {
          deviceType: 2,
  ...
  private parseDeviceMetadata(): DeviceMetadata {
    try {
      ...
      const deviceModel = this.findDeviceModel() || 'Testo 174H';
  ...
  private parseMeasurements(): MeasurementRecord[] {
    ...
    try {
      ...
      // Размер одной записи для двухканального устройства (обычно 12 байт)
      const recordSize = 12;
      ...
      for (let i = 0; i < maxRecords; i++) {
        ...
        try {
          ...
          // Читаем влажность (4 байта, float, little-endian)
          ...
          if (isNaN(temperature) || temperature < -50 || temperature > 100) {
            validationErrors.push('Невалидная температура');
            isValid = false;
          }
          
          // Валидация влажности
          if (isNaN(humidity) || humidity < 0 || humidity > 100) {
            validationErrors.push('Невалидная влажность');
            isValid = false;
          }

          // Если данные невалидны, пропускаем запись
          if (!isValid) {
            continue;
          }
  ...
  private convertTimestamp(rawTimestamp: number): Date {
    try {
      // Для Testo 174H timestamp может быть в разных форматах
