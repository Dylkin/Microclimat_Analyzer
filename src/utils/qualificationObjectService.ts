...
class QualificationObjectService {
  ...
  async deleteLoggerRemovalFile(objectId: string, zoneNumber: number, level: number): Promise<void> {
    ...
    try {
      ...
      const storageFiles: StorageFile[] = (files || []) as StorageFile[];
  ...
  async getLoggerRemovalFiles(objectId: string): Promise<{ [key: string]: { name: string; url: string; size: number; lastModified: string } }> {
    ...
    try {
      ...
      const folders: StorageFile[] = (data || []) as StorageFile[];
      ...
      console.log('QualificationObjectService: Найдены папки в Storage:', folders.map(folder => folder.name));
      ...
      for (const folder of folders) {
        ...
        const storageZoneFiles: StorageFile[] = (zoneFiles || []) as StorageFile[];

        if (zoneError || storageZoneFiles.length === 0) {
          console.log('QualificationObjectService: Нет файлов в папке', folder.name, 'или ошибка:', zoneError);
          continue;
        }

        const latestFile = storageZoneFiles[0];
