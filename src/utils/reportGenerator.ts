  ...
  async generateReport(
    ...
    try {
      ...
      const doc = new Docxtemplater(zip, {
        ...
        linebreaks: true,
        errorLogging: true,
        modules: [this.createImageModule(this.nextImageId)],
        nullGetter: (part) => {
      ...
      // Увеличиваем счетчик изображений для следующего использования
      this.nextImageId++;
      ...
      render: (part: any, options: any) => {
        if (part.module === 'ImageModule' && part.value === 'chart') {
          ...
          if (chartImageData && chartImageData.startsWith('data:image/png;base64,')) {
            ...
            const imageFileName = `image${currentImageId}.png`;
            const relationshipId = `rId${currentImageId + 10000}`;
            
            // Создаем XML для вставки изображения
            const imageXml = `<w:p><w:r><w:drawing><wp:inline distT="0" distB="0" distL="0" distR="0"><wp:extent cx="6096000" cy="2032000"/><wp:effectExtent l="0" t="0" r="0" b="0"/><wp:docPr id="${currentImageId + 20000}" name="График"/><wp:cNvGraphicFramePr><a:graphicFrameLocks xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" noChangeAspect="1"/></wp:cNvGraphicFramePr><a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:nvPicPr><pic:cNvPr id="${currentImageId + 20000}" name="График"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill><a:blip r:embed="${relationshipId}"/><a:stretch><a:fillRect/></a:stretch></pic:blipFill><pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="6096000" cy="2032000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p>`;
            ...
            if (zip) {
              ...
              if (relsContent) {
                ...
                if (!relsXml.includes(imageFileName)) {
