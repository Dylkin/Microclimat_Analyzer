...
export class ReportGenerator {
  ...
  private async createNewReport(
    ...
    // Если есть график, используем библиотеку docx для корректного поворота
    if (chartImageData) {
      return await this.createReportWithDocxLibrary(templateZip, reportData, chartImageData);
    } else {
      // Без графика используем старый метод
  ...
  private async createReportWithDocxLibrary(
    templateZip: JSZip,
    reportData: any,
    chartImageData: string
  ): Promise<Blob> {
    try {
      // Читаем шаблон и заменяем плейсхолдеры (кроме графика)
      const documentXml = await templateZip.file('word/document.xml')?.async('text');
      if (!documentXml) {
        throw new Error('Не найден document.xml в шаблоне');
      }

      // Заменяем все плейсхолдеры кроме {chart}
      let processedXml = this.replacePlaceholders(documentXml, reportData);
      
      // Конвертируем base64 изображение в ArrayBuffer
      const base64Data = chartImageData.split(',')[1];
      const imageBuffer = Uint8Array.from(atob(base64Data), c => c.charCodeAt(0));

      // Создаем новый документ с повернутым изображением
      const doc = new Document({
        sections: [{
          properties: {},
          children: [
            // Добавляем содержимое из шаблона до места вставки графика
            ...this.parseXmlToDocxElements(processedXml.split('{chart}')[0]),
            
            // Вставляем повернутое изображение
            new Paragraph({
              children: [
                new ImageRun({
                  data: imageBuffer,
                  transformation: {
                    width: 400,   // Ширина после поворота (в пикселях)
                    height: 600,  // Высота после поворота (в пикселях)
                    rotation: -90, // Поворот на 90° против часовой стрелки
                  },
                }),
              ],
            ...
            // Добавляем содержимое после графика
            ...this.parseXmlToDocxElements(processedXml.split('{chart}')[1] || ''),
          ],
        }],
      });
      ...
      console.error('Ошибка создания отчета с библиотекой docx:', error);
  ...
  private parseXmlToDocxElements(xmlContent: string): (Paragraph | Table)[] {
    const elements: (Paragraph | Table)[] = [];
    
    try {
      // Простой парсер для основных элементов
      const paragraphs = xmlContent.match(/<w:p[^>]*>.*?<\/w:p>/gs) || [];
      
      paragraphs.forEach(paragraphXml => {
        // Извлекаем текст из параграфа
        const textMatches = paragraphXml.match(/<w:t[^>]*>(.*?)<\/w:t>/gs) || [];
        const text = textMatches.map(match => {
          const textContent = match.replace(/<w:t[^>]*>(.*?)<\/w:t>/s, '$1');
          return textContent;
        }).join('');
        ...
        if (text.trim()) {
          ...
          const isCenter = paragraphXml.includes('w:val="center"');
      ...
      // Обработка таблиц
      const tables = xmlContent.match(/<w:tbl[^>]*>.*?<\/w:tbl>/gs) || [];
      ...
      console.warn('Ошибка парсинга XML содержимого:', error);
      // Добавляем простой параграф с текстом
  ...
  private parseTableXml(tableXml: string): Table | null {
    try {
      const rows = tableXml.match(/<w:tr[^>]*>.*?<\/w:tr>/gs) || [];
      
      const tableRows = rows.map(rowXml => {
        const cells = rowXml.match(/<w:tc[^>]*>.*?<\/w:tc>/gs) || [];
        
        const tableCells = cells.map(cellXml => {
          const textMatches = cellXml.match(/<w:t[^>]*>(.*?)<\/w:t>/gs) || [];
          const cellText = textMatches.map(match => {
            return match.replace(/<w:t[^>]*>(.*?)<\/w:t>/s, '$1');
          }).join('');
      ...
      console.warn('Ошибка парсинга таблицы:', error);
      return null;
    }
  }
