...
export class ReportGenerator {
  ...
  private async captureAndRotateChart(chartElement: HTMLElement): Promise<Uint8Array> {
    try {
      ...
      // Применяем трансформации для поворота на -90 градусов (против часовой стрелки)
      ...
      // Конвертируем blob в Uint8Array для совместимости с docx-templates
      const arrayBuffer = await rotatedBlob.arrayBuffer();
      const uint8Array = new Uint8Array(arrayBuffer);
      
      console.log(`Размеры после поворота: ${rotatedCanvas.width}x${rotatedCanvas.height}`);
