-- НАСТРОЙКА RLS ДЛЯ ПРАВИЛЬНЫХ ТАБЛИЦ
-- Выполните этот скрипт в SQL Editor в Supabase Dashboard

-- 1. Сначала проверим, какие таблицы действительно существуют
DO $$
DECLARE
    table_name text;
...
-- 2. Включаем RLS для существующих таблиц
-- Projects
DO $$
BEGIN
...
-- Qualification object testing periods (правильное название!)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'qualification_object_testing_periods') THEN
        ...
        RAISE NOTICE 'RLS включен для таблицы qualification_object_testing_periods';
    ELSE
        RAISE NOTICE 'Таблица qualification_object_testing_periods не существует, пропускаем';
    END IF;
END $$;

-- Testing period documents
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'testing_period_documents') THEN
        ...
        RAISE NOTICE 'RLS включен для таблицы testing_period_documents';
    ELSE
        RAISE NOTICE 'Таблица testing_period_documents не существует, пропускаем';
    END IF;
END $$;
...
-- 3. Создаем политики для существующих таблиц
DO $$
DECLARE
    table_name text;
    tables_to_check text[] := ARRAY[
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('DROP POLICY IF EXISTS "Users can manage testing periods" ON %I', table_name);
            EXECUTE format('DROP POLICY IF EXISTS "Users can manage testing period documents" ON %I', table_name);
