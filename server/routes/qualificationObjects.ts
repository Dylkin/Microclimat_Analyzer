...
const upsertProjectObjectData = async (
  ...
  const fieldConfig: Record<string, { column: string; json?: boolean }> = {
    ...
    equipmentPlacementSavedAt: { column: 'equipment_placement_saved_at' },
    equipmentPlacementSavedBy: { column: 'equipment_placement_saved_by' },
...
router.get('/', async (req, res) => {
  try {
    ...
    if (project_id && hasProjectDataTable && hasProjectQualificationObjects) {
      ...
      query = `
        SELECT 
          ...
          qo.name,
          qo.object_type,
          COALESCE(pqod.storage_zones, qo.storage_zones) as storage_zones,
          COALESCE(pqod.climate_system, qo.climate_system) as climate_system,
          COALESCE(pqod.temperature_limits, qo.temperature_limits) as temperature_limits,
          COALESCE(pqod.humidity_limits, qo.humidity_limits) as humidity_limits,
          COALESCE(pqod.measurement_zones, qo.measurement_zones) as measurement_zones,
          ...
          COALESCE(pqod.equipment_placement_saved_at, qo.equipment_placement_saved_at) as equipment_placement_saved_at,
          COALESCE(pqod.equipment_placement_saved_by, qo.equipment_placement_saved_by) as equipment_placement_saved_by,
          ...
          COALESCE(pqod.latitude, qo.latitude) as latitude,
          COALESCE(pqod.longitude, qo.longitude) as longitude,
          COALESCE(pqod.geocoded_at, qo.geocoded_at) as geocoded_at,
          COALESCE(pqod.area, qo.area) as area,
          COALESCE(pqod.vin, qo.vin) as vin,
          COALESCE(pqod.registration_number, qo.registration_number) as registration_number,
          COALESCE(pqod.body_volume, qo.body_volume) as body_volume,
          COALESCE(pqod.inventory_number, qo.inventory_number) as inventory_number,
    ...
    const objects = result.rows.map((row: any) => ({
      ...
      equipmentPlacementSavedAt: row.equipment_placement_saved_at ? new Date(row.equipment_placement_saved_at) : undefined,
      equipmentPlacementSavedBy: row.equipment_placement_saved_by || undefined,
    ...
    if (projectId && hasProjectQualificationObjects && hasProjectDataTable) {
      fullResult = await pool.query(
        ...
        SELECT 
          ...
          pqod.equipment_placement_saved_at,
          pqod.equipment_placement_saved_by,
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    const allowedFields = [
      ...
      'equipment_placement_saved_at', 'equipment_placement_saved_by',
    ...
    const projectFieldMap: Record<string, string> = {
      ...
      equipment_placement_saved_at: 'equipmentPlacementSavedAt',
      equipment_placement_saved_by: 'equipmentPlacementSavedBy',
